...
const round = (n, d = 2) => { const num = Number(n); return isNaN(num) ? null : parseFloat(num.toFixed(d)); };
...
const calcRealizedVol = (closes, days = 20) => {
...
export default function App() {
  ...
  const carrySeverities = useMemo(() => ({
    ...
    spread: Math.min(100, Math.max(0, ((4.0 - (marketData.usJapanSpread || 0)) / 3.5) * 100)),
    // Only JPY strengthening (negative USD/JPY change) triggers carry unwinds. 3% JPY gain = 100.
    change: Math.max(0, Math.min(100, (-(marketData.usdJpy1MChange || 0) / 3) * 100)),
    // 20-day annualized vol. Floor 6% (calm), ceiling 25% (Aug 2024 unwind peak).
    vol:    Math.min(100, Math.max(0, (((marketData.jpyRealizedVol || 0) - 6) / 19) * 100)),
    // VIX floor 12 (suppressed), ceiling 40 (severe crisis).
    vix:    Math.min(100, Math.max(0, (((marketData.vixIndex || 0) - 12) / 28) * 100)),
    // HY OAS: 2% = compressed, 10% = crisis.
    ted:    Math.min(100, Math.max(0, (((marketData.tedSpread || 0) - 2) / 8) * 100)),
    // BoJ hikes tighten JPY funding. Ceiling 2% = structural shift in Japanese rate policy.
    boj:    Math.min(100, ((marketData.bojRate || 0) / 2.0) * 100)
  }), [marketData]);

  const carryTradeScore = useMemo(() => {
    const { spread, change, vol, vix, ted, boj } = carrySeverities;
  ...
  const prediction = useMemo(() => {
    ...
    const pceScore = Math.min(100, Math.max(0, ((marketData.corePCE || 0) - 2.0) / 1.5 * 100));
    // Momentum multiplier: rising PCE amplifies pressure, falling PCE dampens it.
    const pceMomentum = (marketData.corePCE || 0) > (marketData.corePCEPrev || 0) ? 1.2 : 0.85;
    // 10Y breakeven inflation: market-implied expectations. 2.0% = 0, 3.5% = 100.
    const breakevenScore = Math.min(100, Math.max(0, ((marketData.breakeven10Y || 0) - 2.0) / 1.5 * 100));
    // Oil supply shock above $90: $90 = 0, $120 = 100.
    const oilScore = Math.min(100, Math.max(0, ((marketData.brentCrude || 0) - 90) / 30 * 100));
    // Tight labor market below NAIRU (~4.5%): wage inflation risk. 3.5% = 100, 4.5% = 0.
    const tightLaborScore = Math.min(100, Math.max(0, (4.5 - (marketData.unemployment || 0)) / 1.0 * 100));
    // Avg Hourly Earnings YoY: above 3.5% = wage-price spiral risk. 3.5% = 0, 5.0% = 100.
    const wageScore = Math.min(100, Math.max(0, ((marketData.wageGrowth || 0) - 3.5) / 1.5 * 100));
    // NFP hot jobs: above trend (150k) = tight labor demand. 150k = 0, 350k = 100.
    const nfpInflScore = Math.min(100, Math.max(0, ((marketData.nfp || 0) - 150) / 200 * 100));
    // JOLTS openings elevated: above 8M = demand still hot. 8M = 0, 12M = 100.
    const joltsInflScore = Math.min(100, Math.max(0, ((marketData.jolts || 0) - 8.0) / 4.0 * 100));
    // Consumer 1Y inflation expectations: unanchored expectations are self-fulfilling. 2.5% = 0, 4.5% = 100.
    const michScore = Math.min(100, Math.max(0, ((marketData.consumerInflExp || 0) - 2.5) / 2.0 * 100));
    // PPI YoY: upstream of PCE by 3–6 months. Signals pipeline inflation before it hits consumers. 2% = 0, 10% = 100.
    const ppiScore = Math.min(100, Math.max(0, ((marketData.ppiYoy || 0) - 2.0) / 8.0 * 100));
    ...
    const unempScore = Math.min(100, Math.max(0, ((marketData.unemployment || 0) - 4.1) / 1.4 * 100));
    // Yield curve inversion (T10Y2Y): +0.5% = 0, flat = 25, -0.5% = 50, -1.5% = 100.
    const yieldCurveScore = Math.min(100, Math.max(0, (0.5 - (marketData.yieldCurve || 0)) / 2.0 * 100));
    // Initial jobless claims: 220k = 0, 500k = 100. Leading labor market indicator.
    const joblessScore = Math.min(100, Math.max(0, ((marketData.joblessClaims || 0) - 220) / 280 * 100));
    // Disinflation: PCE falling below 2.5% = Fed has room to cut. 2.5% = 0, 1.5% = 100.
    const disinfScore = Math.min(100, Math.max(0, (2.5 - (marketData.corePCE || 0)) / 1.0 * 100));
    ...
    const mortgageScore = Math.min(100, Math.max(0, ((marketData.mortgage30Y || 0) - 6.0) / 2.0 * 100));
    ...
    const rateCutPremiumScore = Math.min(100, Math.max(0, ((marketData.fedFundsRate || 0) - (marketData.us2y || 0)) / 1.5 * 100));
    // NFP weak jobs: below trend signals labor softening. 150k = 0, job losses (-50k) = 100.
    const nfpCutScore = Math.min(100, Math.max(0, (150 - (marketData.nfp || 0)) / 200 * 100));
    // JOLTS openings declining: below 8M = demand cooling, leading unemployment by ~6 months. 8M = 0, 5M = 100.
    const joltsCutScore = Math.min(100, Math.max(0, (8.0 - (marketData.jolts || 0)) / 3.0 * 100));
    ...
    const igScore  = Math.min(100, Math.max(0, ((marketData.igSpread  || 0) - 0.80) / 1.70 * 100));
    const bbbScore = Math.min(100, Math.max(0, ((marketData.bbbSpread || 0) - 1.00) / 2.00 * 100));
    ...
    const fsiScore = Math.min(100, Math.max(0, (marketData.financialStressIdx || 0) / 2.0 * 100));
    ...
    let scenario;
    if (inflationPressure >= 55 && cutPressure >= 55) scenario = 'stagflation';
    else if (netScore > 25)   scenario = 'hold';
    else if (netScore >= 0)   scenario = 'wait';
    else if (netScore >= -25) scenario = 'cut_lean';
    else scenario = 'cut';
  ...
  const [trumpCutBias, setTrumpCutBias] = useState(0);
